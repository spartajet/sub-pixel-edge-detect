...
static  void * xmalloc_file(size_t size)
{
	...
	p = malloc(size);
	if (p == NULL) error_file((char *)"out of memory");
	return p;
}
...
{
	if (fclose(f) == EOF) error_file((char *)"unable to close file");
	return 0;
}
...
static void skip_whites_and_comments(FILE * f)
{
	...
	if (c != EOF && ungetc(c, f) == EOF)
		error_file((char *)"unable to 'ungetc' while reading PGM file.");
}
...
static int get_num(FILE * f)
{
	...
	if (!isdigit(c)) error_file((char *)"corrupted PGM or PPM file.");
	num = c - '0';
...
static double * read_pgm_image(char * name, int * X, int * Y)
{
	...
	/* read header */
	if (getc(f) != 'P') error_file((char *)"not a PGM file!");
	if ((n = getc(f)) == '2') bin = FALSE;
	else if (n == '5') bin = TRUE;
	else error_file((char *)"not a PGM file!");
	...
	*Y = get_num(f);               /* Y size */
	...
	if (depth < 0) error_file((char *)"pixel depth < 0, unrecognized PGM file");
	if (bin && depth > 255) error_file((char *)"pixel depth > 255, unrecognized PGM file");
	/* white before data */
	if (!isspace(getc(f))) error_file((char *)"corrupted PGM file.");
...
static double * read_asc_file(char * name, int * X, int * Y)
{
	...
	if (n != 4 || *X <= 0 || *Y <= 0 || Z <= 0 || C <= 0) error_file((char *)"invalid ASC file");

	/* only gray level images are handled */
	if (Z != 1 || C != 1) error_file((char *)"only single channel ASC files are handled");
	...
	{
		n = fscanf(f, "%lf", &val);
		if (n != 1) error_file((char *)"invalid ASC file");
		image[i] = val;
	}
	...
	char * filename, int X, int Y, double width)
{
	...
	/* check input */
	if (filename == NULL) error_file((char *)"invalid filename in write_curves_pdf");
	if (M > 0 && (x == NULL || y == NULL || curve_limits == NULL))
		error_file((char *)"invalid curves data in write_curves_pdf");
	if (X <= 0 || Y <= 0) error_file((char *)"invalid image size in write_curves_pdf");
	...
	char * filename)
{
	...
	/* check input */
	if (filename == NULL) error_file((char *)"invalid filename in write_curves_txt");
	if (M > 0 && (x == NULL || y == NULL || curve_limits == NULL))
		error_file((char *)"invalid curves data in write_curves_txt");
	...
	char * filename, int X, int Y, double width)
{
	...
	/* check input */
	if (filename == NULL) error_file((char *)"invalid filename in write_curves_svg");
	if (M > 0 && (x == NULL || y == NULL || curve_limits == NULL))
		error_file((char *)"invalid curves data in write_curves_svg");
	if (X <= 0 || Y <= 0) error_file((char *)"invalid image size in write_curves_svg");
